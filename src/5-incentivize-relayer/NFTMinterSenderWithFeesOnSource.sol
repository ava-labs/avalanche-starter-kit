...
contract NFTMinterSenderWithFeesOnSource {
	...
	IERC20 feeContract = IERC20(<fee_token_address>);
    ...
    function sendCreateNFTMessage(address destinationAddress, string memory name, string memory symbol) external returns (bytes32 messageID) {
  ...
  function sendMintNFTMessage(address destinationAddress, address to, uint256 tokenId) external returns (bytes32 messageID) {
