...
contract ERC20Source is IERC20Source, TeleporterTokenSource {
     ...
     */
    function addCollateral(bytes32 destinationBlockchainID, address destinationBridgeAddress, uint256 amount)
        external
    {
    ...
    function _handleSendAndCall(SingleHopCallMessage memory message, uint256 amount) internal virtual override {
        ...
        bytes memory payload = abi.encodeCall(
            ...
            (message.sourceBlockchainID, message.originSenderAddress, address(token), amount, message.recipientPayload)
        );
